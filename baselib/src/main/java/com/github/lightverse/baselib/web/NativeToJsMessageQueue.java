...
public class NativeToJsMessageQueue {
    ...
    public void setBridgeMode(int value) {
        if (value < -1 || value >= bridgeModes.size()) {
            CoLog.d(LOG_TAG, "Invalid NativeToJsBridgeMode: " + value);
        } else {
            BridgeMode newMode = value < 0 ? null : bridgeModes.get(value);
            if (newMode != activeBridgeMode) {
                CoLog.d(LOG_TAG, "Set native->JS mode to " + (newMode == null ? "null" : newMode.getClass().getSimpleName()));
                synchronized (this) {
    ...
    public void addPluginResult(PluginResult result, String callbackId) {
        if (callbackId == null) {
            CoLog.e(LOG_TAG, "Got plugin result with no callbackId", new Throwable());
            return;
        }
    ...
    private void enqueueMessage(JsMessage message) {
        synchronized (this) {
            if (activeBridgeMode == null) {
                CoLog.d(LOG_TAG, "Dropping Native->JS message due to disabled bridge");
                return;
            }
            queue.add(message);
            if (!paused) {
    ...
    public void setPaused(boolean value) {
        if (paused && value) {
            ...
            CoLog.e(LOG_TAG, "nested call to setPaused detected.", new Throwable());
        }
        paused = value;
        if (!value) {
            synchronized (this) {
