...
public class CordovaBridge {
    ...
    private JsToNativeManager pluginManager;
    private NativeToJsMessageQueue jsMessageQueue;
    private volatile int expectedBridgeSecret = -1; // written by UI thread, read by JS thread.

    public CordovaBridge(JsToNativeManager pluginManager, NativeToJsMessageQueue jsMessageQueue) {
...
//            CordovaResourceApi.jsThread = Thread.currentThread();
    ...
    private boolean verifySecret(String action, int bridgeSecret) throws IllegalAccessException {
        if (!jsMessageQueue.isBridgeEnabled()) {
            if (bridgeSecret == -1) {
                CoLog.d(LOG_TAG, action + " call made before bridge was enabled.");
            } else {
                CoLog.d(LOG_TAG, "Ignoring " + action + " from previous page load.");
            }
            return false;
        }
        // Bridge secret wrong and bridge not due to it being from the previous page.
        if (expectedBridgeSecret < 0 || bridgeSecret != expectedBridgeSecret) {
            CoLog.e(LOG_TAG, "Bridge access attempt with wrong secret token, possibly from malicious code. Disabling exec() bridge!");
            clearBridgeSecret();
...
//                return ""+secret;
//            } else {
//                CoLog.e(LOG_TAG, "gap_init called from restricted origin: " + origin);
//            }
//            return "";
//        }
        return null;
